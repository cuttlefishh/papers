...
Locus tag & \multicolumn{2}{c}{Counts (rpkm)} & Fold ch. & Gene name and function & Pathway \\\\
\cline{2-3}
...
cat temp2 | grep "deseq2.*noiseq" | grep -v "hypothetical protein" | perl -ne 'chomp; @f = split /\t/; if ($f[4]>0) {$ff = join(" \} & \\textcolor\{blue\}\{ ",@f[0,5,6,4,7,8])} else {$ff = join(" \} & \\textcolor\{red\}\{ ",@f[0,5,6,4,7,8])}; if (($f[5]+$f[6]>=500)&&(abs($f[4])>=0.4)) { if ($f[4]>0) { print "\\textcolor\{blue\}\{ $ff \} \\\\\n"} else { print "\\textcolor\{red\}\{ $ff \} \\\\\n"}}' | sed 's/RNA-/RNA\\us{}/' > temp3
sort -n -r -k14 temp3
