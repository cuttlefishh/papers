...
$usage = qq{
    ...
    noiseqMatchDeg.pl <mynoiseq.deg.csv> <genes.tsv>

	<mynoiseq.deg.csv>: list of differentially expressed genes with 12 fields
	...
	description for each gene.  It prints the resulting 13-field tsv file to standard
	output.
...
foreach my $line (@data1) {
    chomp $line;
    @fields = split(/,/, $line);
    $fields[1] = sprintf("%1.1f", $fields[1]); #round to 3 decimal places
    $fields[2] = sprintf("%1.1f", $fields[2]); #round to 3 decimal places
    $fields[3] = sprintf("%1.3f", $fields[3]); #round to 3 decimal places
    $fields[4] = sprintf("%1.1f", $fields[4]); #round to 3 decimal places
    $fields[5] = sprintf("%1.3f", $fields[5]); #round to 3 decimal places
    $fields[6] = sprintf("%1.1f", $fields[6]); #round to 3 decimal places
    $seqid = $fields[0];
    foreach $linep (@data2) {
	chomp $linep;
