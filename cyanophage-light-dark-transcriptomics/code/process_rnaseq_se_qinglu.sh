...
/opt/gridengine/bin/lx26-amd64/qsub -t 1:${#barcode[@]} ${scriptdir}/qinglu/align_se_cluster_qinglu.sh ${seqfile} ${db} ${gffdir} 

# 5/2/12: having problem with paths on cluster nodes
# qsub -t 1:${#barcode[@]} ${scriptdir}/align_se_cluster_qinglu.sh ${seqfile} ${db} ${gffdir}


echo "Jobs submitted to cluster"
