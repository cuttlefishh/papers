...
$usage = qq{
    ...
    combineKrse2011HumannTaxaByKO.pl <physchemdata> <humannoutput> <outfile>
        ...
        "ByKO" variation lists strain for each column header, not KO (line 54).
...
foreach $line (@data2) {
    ...
    if ($lineno >= 1) { # start at line 1 (not 15) bc taxa-specific data has no header material
        ...
        @fields = split(/\t/,$line);
        $index = 0;
        $hash_humann{$index} .= ",$fields[2]"; # index 0 => 0:short_name, 1:long_name, 2:genome
        for ($i=2; $i<=91; $i++) {
            $index = $i-1;
