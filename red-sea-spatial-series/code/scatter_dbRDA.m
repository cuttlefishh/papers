...
for i = 7:8 % 1:size(files,1)
    ...
    for j = 1:size(predictor,1)
        ph = plot([0 pcoord1(j)*multiplier], [0 pcoord2(j)*multiplier], '-r.', 'Color', cbSetOne9(9,:), 'Marker', 'none');
        if (pcoord2(j) > 0)
            text(pcoord1(j)*multiplier, pcoord2(j)*multiplier, predictor(j), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
        else
            text(pcoord1(j)*multiplier, pcoord2(j)*multiplier, predictor(j), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'top');
        end
    end
    
    % add circle
    %t = linspace(0,2*pi);
    %ph = plot(cos(t)*multiplier,sin(t)*multiplier);
    ...
    label1 = ['dbRDA1 (', num2str(totalIndiv(1)), '%)'];
    label2 = ['dbRDA2 (', num2str(totalIndiv(2)), '%)'];
    xlabel(label1); % depth
